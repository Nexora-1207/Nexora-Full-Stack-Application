...
import { SECTOR_TREES, SectorTree } from '@/lib/sectorTrees';
...
export default function SectorDetailClient({ sectorId }: Props) {
  const router = useRouter();
  const normalizedKey = sectorId.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-');
  const sectorData: SectorTree = SECTOR_TREES[normalizedKey] || 
    SECTOR_TREES[sectorId.toLowerCase()] || 
    Object.values(SECTOR_TREES).find(s => 
      s.id.toLowerCase() === sectorId.toLowerCase() || 
