        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 shrink-0 pb-3 border-b border-slate-200 dark:border-white/[0.08]">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
            ...
            <button
              ...
              className="flex-1 sm:flex-none min-h-[44px] py-2.5 px-3 rounded-2xl bg-cyber-violet/20 hover:bg-cyber-violet/30 border border-cyber-violet/40 text-cyber-violet font-black text-[11px] sm:text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 transition shadow-md active:scale-95 text-center shrink-0"
            >
            ...
            <button
              ...
              className="flex-1 sm:flex-none min-h-[44px] cyber-button-primary py-2.5 px-3.5 rounded-2xl text-[11px] sm:text-xs font-black uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-lg active:scale-95 text-center shrink-0"
            >
              <Upload className="w-4 h-4 shrink-0 text-background" />
              <span>+ UPLOAD DOCUMENT</span>
            </button>
          </div>
        </div>

      {/* STORAGE METRICS & COMPACT SEARCH BAR */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 shrink-0">
        
        {/* Dynamic Storage Gauge */}
        <div className="glass-panel rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 flex flex-col justify-between space-y-2.5 shadow-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-cyber-cyan/10 border border-cyber-cyan/30 flex items-center justify-center text-cyber-cyan shrink-0">
                <HardDrive className="w-4 h-4 sm:w-5 sm:h-5" />
              </div>
              <div>
                <h3 className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white">
                  VAULT STORAGE
                </h3>
            ...
            <span className="text-[10px] sm:text-xs font-black text-cyber-cyan font-mono bg-cyber-cyan/10 px-2 py-0.5 rounded-lg border border-cyber-cyan/20">
              {memoryPercent}% USED
            </span>
          </div>

          <div className="space-y-1">
            <div className="w-full h-2 sm:h-2.5 rounded-full bg-slate-200 dark:bg-white/[0.06] overflow-hidden p-0.5 border border-slate-300 dark:border-white/[0.08]">
              <div 
            ...
            <div className="flex items-center justify-between text-[9px] sm:text-[10px] font-bold text-slate-500 dark:text-white/50 font-mono">
        ...
        {/* Category Filter Pills & Icon Search Button */}
        <div className="lg:col-span-2 glass-panel rounded-2xl sm:rounded-3xl p-3 sm:p-5 flex flex-col justify-center space-y-2.5 shadow-lg">
          
          {/* Top Control Bar: Category Pills + Search Icon Button */}
          <div className="flex items-center justify-between gap-2">
            
            {/* Category Tabs */}
            <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none flex-1 pr-1">
              {CATEGORIES.map((cat) => (
                <button
                  key={cat}
                  onClick={() => setActiveCategory(cat)}
                  className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-wider transition shrink-0 ${
                    activeCategory === cat
            ...
            <button
              ...
              className={`min-h-[36px] px-3 py-1.5 rounded-xl border flex items-center gap-1.5 text-xs font-black transition shrink-0 active:scale-95 ${
                isSearchOpen || searchQuery
                  ? 'bg-cyber-cyan text-background border-cyber-cyan shadow-md shadow-cyber-cyan/20'
                  : 'bg-slate-100 dark:bg-white/[0.05] border-slate-200 dark:border-white/[0.1] text-slate-700 dark:text-white hover:bg-slate-200 dark:hover:bg-white/[0.1]'
              }`}
          ...
          {/* Expandable Search Bar Input Box (Only visible when search icon clicked or query present) */}
          {(isSearchOpen || searchQuery) && (
            <div className="relative animate-in fade-in slide-in-from-top-2 duration-200 pt-0.5">
              <Search className="w-4 h-4 text-cyber-cyan absolute left-3.5 top-2.5" />
              <input
                type="text"
                value={searchQuery}
                ...
                autoFocus
                className="w-full bg-slate-100 dark:bg-surface-card border border-cyber-cyan/40 rounded-xl pl-10 pr-9 py-2 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-white/30 focus:outline-none focus:border-cyber-cyan transition font-medium shadow-inner"
              />
              <button
                onClick={() => {
                  setSearchQuery('');
                ...
                className="absolute right-2.5 top-2 p-1 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-white transition"
              >
