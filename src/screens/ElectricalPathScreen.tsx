...
const YELLOW_DARK  = '#FFEE00';
const YELLOW_LIGHT = '#B8A800';

// ─── DATA ────────────────────────────────────────────────────────────────────

const WHY_OPTIONS = [
  { id: 'grid',    text: 'I am fascinated by power grids, transformers & high voltage' },
  { id: 'solar',   text: 'I want to build Solar & Wind renewable energy systems' },
  { id: 'psu',     text: 'I seek elite PSU jobs (PowerGrid / NTPC / BHEL)' },
  { id: 'ev',      text: 'I want to engineer EV charging networks & microgrids' },
  { id: 'circuits',text: 'I love electromagnetism & electrical circuit math' },
  { id: 'explore', text: 'I am exploring electrical engineering paths' },
];

const WHAT_CARDS = [
  { icon: 'lightning-bolt',        label: 'Power Systems & Grid' },
  { icon: 'solar-power',           label: 'Solar & Wind Energy' },
  { icon: 'transmission-tower',    label: 'High Voltage Trans' },
  { icon: 'home-lightning-bolt',   label: 'Substation Automation' },
  { icon: 'engine',                label: 'Electrical Machines' },
  { icon: 'counter',               label: 'Smart Microgrids' },
];

const CHECKLIST_Q = [
  'Do you enjoy studying Physics electricity, magnetic fields, and circuit equations?',
  'Are you curious about how national power grids distribution and substations operate?',
  'Are you excited about renewable solar, wind, and green energy technologies?',
  'Would you like to prepare for GATE to join elite PSUs like PowerGrid, NTPC, or BHEL?',
  'Can you follow strict high-voltage safety standards and electrical codes?',
  'Do you want a stable, essential core engineering career with high impact?',
];

const PERSONALITY = [
  { skill: 'Circuit Mathematics',icon: 'lightning-bolt',        why: 'Analyze AC/DC power flow, 3-phase circuits, and electromagnetic fields.' },
  { skill: 'High-Voltage Safety',icon: 'shield-alert-outline',  why: 'Adhere strictly to high-voltage insulation and substation safety protocols.' },
  { skill: 'Renewable Passion', icon: 'solar-power',           why: 'Design solar PV plant arrays, wind turbine generators, and battery storage.' },
  { skill: 'Grid Protection',   icon: 'shield-check-outline',  why: 'Program relay protection systems, circuit breakers, and fault isolations.' },
  { skill: 'System Automation', icon: 'chip',                  why: 'Implement SCADA, PLC controllers, and smart digital substation networks.' },
  { skill: 'Problem Solving',   icon: 'puzzle-outline',        why: 'Diagnose power dropouts, transformer harmonics, and load balancing.' },
];
...
const CHECKPOINTS = [
  {
    ...
    icon: 'school-outline',
    detail: 'Build strong fundamentals in Physics, Mathematics, and electricity in Class 10.',
  },
  {
    id: 2, label: 'Choose 11th & 12th (MPC) or Electrical Diploma',
    icon: 'book-outline',
    detail: 'Physics, Chemistry, Mathematics (MPC) in 10+2 or a 3-year Polytechnic Diploma in Electrical Engineering.',
    sub: ['Physics', 'Chemistry', 'Mathematics', 'Electrical Diploma'],
    note: '⚡ MPC stream is mandatory for B.Tech EEE / Electrical degree admissions.',
  },
  {
    id: 3, label: 'JEE Main & GATE Preparation',
    icon: 'sparkles-outline',
    detail: 'Clear national entrance exams for B.Tech and prepare for GATE for PSU recruitment.',
    sub: ['JEE Main', 'JEE Advanced', 'GATE EEE', 'State CETs'],
  },
  {
    id: 4, label: "Bachelor's Degree in Electrical (B.Tech / B.E.)",
    icon: 'medal-outline',
    detail: 'Enroll in a 4-year B.Tech Electrical / EEE degree.',
    degrees: [
      { name: 'B.Tech Electrical & Electronics (EEE)', dur: '4 yrs', subjects: 'Power Systems, Control Systems, Electrical Machines, Microcontrollers, Power Electronics', careers: 'Electrical Design Engineer, Power Analyst' },
      { name: 'B.Tech Electrical Engineering', dur: '4 yrs', subjects: 'High Voltage Tech, Switchgear, Protection, Generation, Transformers', careers: 'Substation Engineer, Protection Lead' },
      { name: 'B.Tech Renewable Energy Engg', dur: '4 yrs', subjects: 'Solar PV Systems, Wind Energy Tech, Energy Storage, Smart Grids', careers: 'Solar Plant Designer, Renewable Specialist' },
      { name: 'Polytechnic Diploma Electrical', dur: '3 yrs', subjects: 'Electrical Wiring, Motor Maintenance, Distribution Lines', careers: 'Junior Engineer (JE), Substation Operator' },
    ],
  },
  {
    id: 5, label: 'Substation & Solar Plant Internships',
    icon: 'briefcase-outline',
    detail: 'Complete 2-4 month hands-on internships at state electricity boards, solar power plants, or transformer manufacturing units.',
  },
  {
    id: 6, label: 'GATE Qualification & PSU Interviews',
    icon: 'ribbon-outline',
    detail: 'Achieve a top GATE score for direct recruitment into Navratna/Maharatna PSUs.',
  },
  {
    id: 7, label: 'PSU & MNC Electrical Recruitment',
    icon: 'briefcase-check-outline',
    detail: 'Join premier power sector corporations and engineering conglomerates.',
    companies: ['PowerGrid (PGCIL)', 'NTPC', 'BHEL', 'Tata Power', 'Schneider Electric', 'Siemens', 'ABB'],
  },
  {
    id: 8, label: 'Career Growth to Chief Power Systems Architect',
    icon: 'trending-up',
    detail: 'Advance to Chief Electrical Engineer, PSU Executive Director, or Renewable Energy Leader.',
    growth: ['Junior Engineer Trainee', 'Electrical Design Engineer', 'Substation Manager', 'Chief Grid Engineer', 'Executive Director / VP Power'],
  },
];

const SKILLS = [
  { icon: 'lightning-bolt',             skill: 'Power Systems Analysis (ETAP/MATLAB)' },
  { icon: 'solar-power',                skill: 'Solar PV & Wind Plant Design' },
  { icon: 'transmission-tower',         skill: 'High Voltage Transmission' },
  { icon: 'shield-check-outline',       skill: 'Switchgear & Relay Protection' },
  { icon: 'chip',                       skill: 'PLC & SCADA Substation Automation' },
  { icon: 'engine',                     skill: 'Transformer & Motor Overhaul' },
  { icon: 'home-lightning-bolt',        skill: 'EV Charging Microgrid Design' },
  { icon: 'shield-alert-outline',       skill: 'Electrical Safety Codes (IE Rules)' },
];

const CAREERS = [
  { role: 'Power Systems Engineer', icon: 'lightning-bolt',        what: 'Designs high-voltage transmission networks, load flow calculations, and grid stability.', skills: 'ETAP, Load Flow, Grid Protection, MATLAB' },
  { role: 'Solar / Renewable Specialist', icon: 'solar-power',     what: 'Engineers commercial solar farms, wind turbine generators, and battery storage integration.', skills: 'PVsyst, Solar Inverters, Grid Tie' },
  { role: 'Substation Automation Lead', icon: 'home-lightning-bolt',what: 'Programs SCADA, PLC switchgears, digital meters, and substation remote control.', skills: 'SCADA, PLC, Switchgear, IEC 61850' },
  { role: 'PSU Electrical Officer',  icon: 'transmission-tower',   what: 'Manages state/national power generation plants, transmission lines, and grid operations.', skills: 'Power Generation, PGCIL Rules, Maintenance' },
  { role: 'Power Electronics Engineer', icon: 'chip',             what: 'Designs inverters, EV motor drives, DC-DC converters, and uninterruptible power units.', skills: 'Power MOSFETs, IGBTs, Inverters, Simulation' },
  { role: 'Electrical Design Specialist', icon: 'draw',            what: 'Creates commercial building single-line diagrams (SLD), panel layouts, and earthing designs.', skills: 'AutoCAD Electrical, SLD, Earthing, Panels' },
];

const CHALLENGES = [
  { icon: 'shield-alert-outline',       title: 'High Voltage Hazards',      desc: 'Working with multi-kilovolt substations requires uncompromising safety focus.' },
  { icon: 'transmission-tower',         title: 'Remote Plant Posting',       desc: 'Thermal and solar power plants are frequently situated in remote geographical areas.' },
  { icon: 'lightning-bolt-outline',     title: 'Emergency Outage Response', desc: 'Grid blackouts or transformer faults require fast emergency restoration.' },
];

const COLLEGES = [
  { name: 'IIT Kharagpur',       city: 'Kharagpur', desc: 'India’s premier institute famous for electrical engineering research & power systems.' },
  { name: 'IIT Roorkee',         city: 'Roorkee',   desc: 'Historic leader in hydro power, electrical machines, and high voltage labs.' },
  { name: 'NIT Trichy',          city: 'Trichy',    desc: 'Top NIT with outstanding GATE cutoffs and PSU campus placements.' },
  { name: 'CEG Anna University', city: 'Chennai',   desc: 'Renowned Southern university with cutting-edge high voltage & solar research.' },
  { name: 'VJTI Mumbai',         city: 'Mumbai',    desc: 'Top choice for power electronics, switchgear, and substation automation.' },
  { name: 'BITS Pilani',         city: 'Pilani',    desc: 'State-of-the-art EEE laboratories, microgrid incubators, and corporate ties.' },
];

const SALARY_TIERS = [
  { level: 'Graduate Engineer Trainee (GET)', range: '₹4.5–8 LPA' },
  { level: 'Electrical Engineer / Specialist', range: '₹8–15 LPA' },
  { level: 'Senior Substation / PSU Lead', range: '₹15–25 LPA' },
  { level: 'Chief Grid Architect / DGM',  range: '₹25–42 LPA' },
  { level: 'PSU Executive Director / VP', range: '₹60+ LPA' },
];

const SUCCESS_TIPS = [
  'Master Kirchhoff laws, AC circuit phasors, and electromagnetism during 11th & 12th.',
  'Master ETAP, MATLAB Simulink, and AutoCAD Electrical during your B.Tech EEE.',
  'Prepare for GATE EEE starting from 3rd year to secure top PSU job calls.',
  'Learn solar PV plant design software (PVsyst) and SCADA/PLC programming.',
  'Complete internships at state electricity transmission boards or solar companies.',
  'Understand national electrical codes, earthing standards, and safety insulation.',
  'Follow the global transition toward smart grids, microgrids, and EV charging stations.',
];
...
const ElectricalPathScreen = ({ navigation }: any) => {
  ...
  const C = isDark ? YELLOW_DARK : YELLOW_LIGHT;

  useEffect(() => {
    const saveSector = async () => {
      try {
        await AsyncStorage.setItem('activeSector', 'ELECTRICAL');
      } catch (_) {}
    };
    saveSector();
  }, []);
  ...
  return (
    <SafeAreaView style={[s.root, { backgroundColor: bg }]}>
      ...
      <View style={s.navWrap}>
        <TopNavBar title="Electrical & Power Career Guide" />
      ...
      >
        ...
        >
          ...
          >
            ...
            <MaterialCommunityIcons name="lightning-bolt" size={68} color={C} />
          </LinearGradient>

          <Text style={[s.heroTitle, { color: txt }]}>Electrical & Power Career Roadmap</Text>
          <Text style={[s.heroSub, { color: sub }]}>
            Master B.Tech Electrical & Electronics (EEE), Power Systems, Solar Energy, Substation Automation, and PSUs after Class 10.
          </Text>
        ...
        <Section title="Before We Begin..." color={C} textColor={txt}>
          <Text style={[s.sectionSub, { color: sub }]}>
            Let's evaluate your passion for power grids, renewable energy, and electrical systems.
          </Text>
          <GlassCard bg={card} border={bdr}>
            <Text style={[s.questionLabel, { color: txt }]}>💬 Why choose Electrical & Power Systems?</Text>
            ...
            {whySelected.length > 0 && (
              ...
              >
                ...
                <Text style={[s.feedbackTxt, { color: isDark ? '#FFF' : '#005C5C' }]}>
                  {'  '}Awesome! Electrical Engineering powers civilization, driving the transition to solar energy, smart grids, and EVs.
                </Text>
              </LinearGradient>
            )}
          </GlassCard>
        ...
        {/* SECTION 3 – WHAT IS ELECTRICAL ENGINEERING? */}
        <Section title="What is Electrical Engineering?" color={C} textColor={txt}>
          <Text style={[s.sectionSub, { color: sub }]}>
            Electrical Engineering encompasses generation, high-voltage grid transmission, solar energy systems, substations, and power electronics.
          </Text>
        ...
        {/* SECTION 4 – IS ELECTRICAL RIGHT FOR YOU? */}
        <Section title="Is Electrical Right For You?" color={C} textColor={txt}>
          <GlassCard bg={card} border={bdr}>
            ...
            {answered === 6 && (
              ...
              >
                ...
                <Text style={[s.resultTxt, { color: txt }]}>
                  {' '}{yesCount >= 4
                    ? '⚡ You have an ideal analytical mindset for electrical & power systems engineering!'
                    : '💪 Circuit and power engineering skills can be developed through university programs.'}
                </Text>
              </LinearGradient>
            )}
          </GlassCard>
        ...
        <Section title="Personality Match" color={C} textColor={txt}>
          <Text style={[s.sectionSub, { color: sub }]}>Key traits that excel in electrical engineering:</Text>
        ...
        <Section title="Challenges You Should Know" color={C} textColor={txt}>
          <Text style={[s.sectionSub, { color: sub }]}>Realities of an Electrical Engineering career:</Text>
          {CHALLENGES.map((ch, i) => (
        ...
        <Section title="Top Electrical Institutes" color={C} textColor={txt}>
          {COLLEGES.map((col, i) => (
        ...
        <Section title="Salary Overview" color={C} textColor={txt}>
          <GlassCard bg={card} border={bdr}>
            ...
            <Text style={[s.salaryNote, { color: sub }]}>
              * PSU officers, solar project leads, and power grid engineers enjoy high stability and attractive benefits.
            </Text>
          </GlassCard>
        </Section>
        ...
        <View style={[s.sectionWrap, { marginBottom: 50 }]}>
          ...
          >
            ...
            <Text style={[s.motTitle, { color: txt }]}>
              Power The Smart Clean Energy Grids of Tomorrow
            </Text>
            <Text style={[s.motBody, { color: sub }]}>
              Electrical engineers generate the clean power driving industries, electric vehicles, and modern cities. Master circuit science and energize the world.
            </Text>
            <TouchableOpacity
