...
const ACCENT_DARK  = '#F0FF00';
const ACCENT_LIGHT = '#B8A000';
...
const CAREER_TREE: any = {
  root: {
    ...
    options: [
      { id: 'inter', label: 'INTERMEDIATE (10+2)', sub: 'Academic Path', next: 'inter_stream', brief: 'Complete 11th & 12th to unlock hotel management degree programs and culinary arts institutions.' },
      { id: 'diploma', label: 'HOSPITALITY DIPLOMA', sub: 'Technical Mastery (1-3 Yrs)', next: 'diploma_branch', brief: 'Focused diploma programs for front office, food production, and catering operations.' },
      { id: 'cert', label: 'CERTIFICATE COURSE', sub: 'Short Skill Programs', next: 'cert_note', brief: 'Quick certifications in housekeeping, food service, bartending, or travel desk operations.' },
    ]
  },
  inter_stream: {
    question: 'DEGREE TARGET',
    subtitle: 'Choose your higher education program',
    options: [
      { id: 'bhmct', label: 'B.HMCt / BHM', sub: 'Hotel Management Degree (3-4 Yrs)', next: 'success', brief: 'Top qualification for hotel operations management, F&B management, and hospitality chains.' },
      { id: 'bsc_hospitality', label: 'B.SC HOSPITALITY', sub: 'Science in Hospitality (3 Yrs)', next: 'success', brief: 'Combines food science, nutrition, and service management for food tech and airline catering.' },
      { id: 'ba_tourism', label: 'BA TRAVEL & TOURISM', sub: 'Tourism Management (3 Yrs)', next: 'success', brief: 'Study tour operations, destination management, event planning, and travel logistics.' },
    ]
  },
  diploma_branch: {
    question: 'DIPLOMA BRANCH',
    subtitle: 'Select your hospitality specialization',
    options: [
      { id: 'food_prod', label: 'FOOD PRODUCTION', sub: 'Culinary Arts & Kitchen Ops', next: 'success', brief: 'Learn classical and modern cooking techniques, bakery, and professional kitchen management.' },
      { id: 'front_office', label: 'FRONT OFFICE OPS', sub: 'Reception & Guest Services', next: 'success', brief: 'Master check-in/check-out operations, reservations, concierge, and guest relationship management.' },
      { id: 'housekeeping', label: 'HOUSEKEEPING', sub: 'Rooms Division Management', next: 'success', brief: 'Manage hotel room standards, laundry, linen, and hygiene protocols for luxury properties.' },
      { id: 'fnb', label: 'FOOD & BEVERAGE SERVICE', sub: 'Restaurant & Bar Operations', next: 'success', brief: 'Cover fine dining service, wine knowledge, banqueting, and restaurant floor management.' },
    ]
  },
  cert_note: {
    type: 'info',
    question: 'CERTIFICATE PATHWAYS',
    text: 'HOSPITALITY CERTIFICATE PROGRAMS:\nShort programs offering immediate job entry in hotels, airlines, and events.\n\nPOPULAR OPTIONS:\n- Housekeeping Certificate (3 Mo)\n- Food Service & Waitering (3-6 Mo)\n- Bartending & Mixology (3 Mo)\n- Travel Desk Operations (6 Mo)\n- Event Management Assistant (6 Mo)\n\nCOURSES OFFERED BY:\n- State Tourism Departments\n- NSDC Hospitality Skill Centers\n- IHCL / Taj Hotels Training Programs\n- ITC Hotels Apprenticeship\n\nSALARY:\n- Entry: ₹1.5 – 2.5 LPA\n- 5-Star Hotels: ₹2.5 – 4.5 LPA\n- Tips + Service Charge can double income',
    next: 'success'
  },
  success: {
...
const HospitalityPathScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    const saveSector = async () => {
      try { await AsyncStorage.setItem('activeSector', 'HOSPITALITY'); } catch (_) {}
    };
    saveSector();
  }, []);
      ...
      Animated.parallel([
        ...
        if (nextKey === 'success') {
          supabase.auth.getUser().then(({ data: { user } }) => {
            if (user) {
              supabase.from('profiles').upsert({ id: user.id, sector: 'HOSPITALITY', updated_at: new Date() })
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTag}>HOSPITALITY</Text>
      </View>
...
const getStyles = (isDark: boolean) => StyleSheet.create({
  ...
  floatingCard: { width: cardWidth, backgroundColor: isDark ? 'rgba(10,10,20,0.98)' : '#FFF', borderRadius: 25, borderWidth: 1, borderColor: isDark ? 'rgba(240,255,0,0.1)' : 'rgba(0,0,0,0.05)', padding: 25, shadowColor: '#000', shadowOffset: { width: 0, height: 15 }, shadowOpacity: isDark ? 0.9 : 0.1, shadowRadius: 30, elevation: isDark ? 20 : 10 },
  ...
  briefingView: { marginTop: 15, borderTopWidth: 1, borderTopColor: isDark ? 'rgba(240,255,0,0.1)' : 'rgba(0,0,0,0.05)', paddingTop: 10 },
